...
public class UnityChan2DController : MonoBehaviour
{
    ...
    private  StateList m_state = StateList.Normal;

    void _state()
    {
        if (m_state == StateList.Normal)
        {
            state = 1;

        }

        if (m_state == StateList.Damaged)
        {
            state = 2;
    ...
    void Awake()
    {
        ...
        unichanhp = GameObject.Find("unichanHP").GetComponent<Slider>();
    }
    ...
    void Update()
    {
        ...
        GetComponent<Animator>().SetBool("Attack1", Attack1);

        
        isGround = m_isGround;
        unichanhp.value = MainHP;
        _state();
    ...
    {
        if (other.tag == "DamageObject" && m_state == StateList.Normal)
        {
            m_state = StateList.Damaged;
    ...
    IEnumerator INTERNAL_OnDamage()
    {
        ...
        m_state = StateList.Invincible;
    }

    void OnFinishedInvincibleMode()
    {
        m_state = StateList.Normal;
    }
